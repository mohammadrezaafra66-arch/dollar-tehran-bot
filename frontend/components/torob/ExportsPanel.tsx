...
export default function TorobExportsPanel() {
  ...
  async function load() {
    setLoading(true);
    try { const res = await api.torob.exports(); setItems(res.items); }
  ...
  async function exportNow() {
    setLoading(true);
    try {
      const res = await api.torob.export();
  ...
  return (
    <div className="bg-white rounded-xl border p-5" dir="rtl">
      <div className="flex items-center justify-between mb-4">
        <h2 className="font-bold text-lg">خروجی‌های Excel ترب</h2>
        ...
        : <div className="space-y-2">
            {items.map(item => (
              <div key={item.name} className="flex items-center justify-between p-3 border rounded-lg">
                ...
                <a href={`/api/torob/exports/${item.name}`} download
