        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="bg-gray-50 text-xs text-gray-500">
              <tr>
                {["#","عنوان","فروشنده","تلفن","شهر","قیمت","وضعیت","پیام","منبع"].map(h => (
            ...
            <tbody>
              {items.length === 0 ? (
                ...
                <tr key={lead.id} className="border-t hover:bg-gray-50">
                  ...
                  <td className="px-3 py-2 max-w-[200px] truncate">{lead.title}</td>
                  ...
                  <td className="px-3 py-2">{lead.price_text}</td>
                  <td className="px-3 py-2">
                    <span className={`text-xs px-2 py-0.5 rounded-full ${STATUS_COLORS[lead.extraction_status] ?? "bg-gray-100"}`}>
                      {lead.extraction_status}
                    </span>
                  </td>
                  <td className="px-3 py-2">{lead.message_sent ? "✅" : "—"}</td>
                  <td className="px-3 py-2">
