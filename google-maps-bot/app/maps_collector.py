...
def collect_businesses(search_query, max_scrolls=None, max_businesses=None):
    """
    جمع‌آوری لیست بیزینس‌ها از گوگل مپ
    
    Args:
        search_query: عبارت جستجو
        max_scrolls: اگر None باشد از Config.MAX_SCROLLS استفاده می‌کند
        max_businesses: اگر None باشد از Config.MAX_BUSINESSES_PER_QUERY استفاده می‌کند
    """
    # اگر مقادیر مشخص نشده، از Config بگیر
    ...
    with sync_playwright() as p:
        ...
        for link in links:
            try:
                ...
                # اگر به حد مجاز رسیدیم، متوقف شو
