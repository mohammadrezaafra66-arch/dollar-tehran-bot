# app/database.py - نسخه کامل با migration خودکار
import sqlite3
...
class Database:
    ...
    def _init_tables(self):
        with self._get_connection() as conn:
            ...
            # Migration برای دیتابیس‌های قدیمی/نیمه‌ساخته‌شده
            self._ensure_column(cursor, 'businesses', 'data_source', "TEXT DEFAULT 'google_maps'")
            self._ensure_column(cursor, 'businesses', 'last_seen_at', 'TIMESTAMP')
            self._ensure_column(cursor, 'businesses', 'raw_href', 'TEXT')
            ...
            self._ensure_column(cursor, 'website_extractions', 'products', 'TEXT')
            self._ensure_column(cursor, 'website_extractions', 'brands', 'TEXT')
            ...
            cursor.execute('CREATE INDEX IF NOT EXISTS idx_businesses_data_source ON businesses(data_source)')
            ...
            for biz in businesses:
                ...
                if existing:
                    ...
                    if update_phone or update_website or update_address:
                        ...
                        cursor.execute('''
                            UPDATE businesses
                            SET phone = ?, website = ?, address = ?, raw_href = ?,
                                last_seen_at = CURRENT_TIMESTAMP, updated_at = ?
                            WHERE id = ?
                        ''', (final_phone, final_website, final_address, raw_href, datetime.now().isoformat(), business_id))
                        print(f"  🔄 Updated business: {str(name)[:40]}...")
                    else:
                        cursor.execute('''
                            UPDATE businesses
                            SET last_seen_at = CURRENT_TIMESTAMP, raw_href = ?, updated_at = ?
                            WHERE id = ?
                        ''', (raw_href, datetime.now().isoformat(), business_id))
                else:
                    cursor.execute('''
                        INSERT INTO businesses
                        (query_id, name, slug, clean_href, raw_href, phone, website, address,
                         status, data_source, last_seen_at, updated_at)
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'pending', 'google_maps', CURRENT_TIMESTAMP, ?)
                    ''', (query_id, name, slug, clean_href, raw_href, phone, website, address, datetime.now().isoformat()))
                    print(f"  ✅ New business added: {str(name)[:40]}...")
    ...
    def mark_success(self, business_id: int, phone: str = '', website: str = '', address: str = ''):
        with self._get_connection() as conn:
            ...
            cursor.execute('''
                ...
                WHERE id = ?
            ''', (phone, website, address, datetime.now().isoformat(), datetime.now().isoformat(), business_id))
